...
import { QuizConfig, DifficultyKey } from '../types/quiz.types';
...
export const DifficultyScreen: React.FC<Props> = ({ quiz, onSelect, onBack }) => {
  const entries = Object.entries(quiz.difficulties) as [
    DifficultyKey,
    QuizConfig['difficulties']['easy']
  ][];
        ...
        {entries.map(([key, cfg], i) => (
          ...
          >
            ...
            <div className="relative z-10 flex flex-col items-center gap-2 sm:gap-3">
              ...
              <div className="text-white/90 text-xs sm:text-sm space-y-1 text-center">
                <div>❓ {cfg.questionsPerGame} perguntas</div>
                <div>⏱️ {cfg.timePerQuestion}s por pergunta</div>
                <div>⭐ {cfg.pointsPerQuestion} pts por acerto</div>
              </div>
            </div>
          </button>
        ))}
